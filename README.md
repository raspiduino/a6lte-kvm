...
- <s>The kernel only detect 4 cores / 8 cores of the Exynos 7870 SoC. I will try to fix this. In the log I can found some lines about `failed to hotplug cpu 4`. Maybe there is something I turn of by mistake when `make menuconfig`. You might not face this.</s> EDIT: Fixed in latest commit thanks to [Henris3330](https://github.com/Henris3330), see [this](https://github.com/sleirsgoevy/exynos-kvm-patch/issues/2).
...
- Step 5: Goto `Boot options` and then turn off all options which has `TIMA` and `RKP`. CPU hotplug will not work, so you have to disable it by disable *all* hotplug config (you might want to open the `.config` in editor and edit it manually). Then click exit to go back to main menu. Enable `Virtualization` by pressing the space key. Goto `Virtualization` menu and enable `Kernel-based Virtual Machine (KVM) support`. After that, exit menuconfig and save `.config` file.
